...
EXPERIMENT_NAME = "transformer_weighted_macro"
...
class TransformerPitchModel(nn.Module):
    def __init__(
        self,
        num_pitch_tokens,
        num_zone_tokens,
        ...
        input_dim = PITCH_EMB_DIM + ZONE_EMB_DIM
    ...
    def forward(self, seq_pitch, seq_zone, numeric, pitcher, stand, p_throws):
        batch_size, seq_len = seq_pitch.shape

        pitch_emb = self.pitch_emb(seq_pitch)
        zone_emb = self.zone_emb(seq_zone)

        x = torch.cat([pitch_emb, zone_emb], dim=-1)
        x = self.input_proj(x)

        positions = torch.arange(seq_len, device=seq_pitch.device).unsqueeze(0)
        x = x + self.pos_emb(positions)

        # True인 위치를 Transformer가 무시함
        pad_mask = seq_pitch.eq(self.pitch_pad_idx)
        ...
        seq_repr = (x * valid_mask).sum(dim=1) / denom
